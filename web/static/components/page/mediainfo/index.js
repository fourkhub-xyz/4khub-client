...
export class PageMediainfo extends CustomElement {
  static properties = {
    ...
    pagefrom: { attribute: "pagefrom" },
      fourkhub_id:{ attribute: "fourkhub_id" },
      coins: { attribute: "coins" },
    is_charge: {attribute: "is_charge"},
    // TMDBID/DB:豆瓣ID
    ...
    return html`
      ...
      <div class="container-xl placeholder-glow page-wrapper-top-off lit-media-info-page-bg">
              ...
              <div class="d-flex justify-content-center">
                <div class="d-flex flex-column justify-content-end ms-2 mt-2">
                  ...
                  <div class="align-self-center align-self-md-start">
                    <a href="${this.media_info.link}" target="_blank" ?hidden=${!this.media_info.tmdbid}><span class="badge badge-outline text-blue">TMDB</span></a>
                    <a href="${this.media_info.douban_link}" target="_blank" ?hidden=${!this.media_info.douban_id}><span class="badge badge-outline text-green">豆瓣</span></a>
                    
                    ${this.is_charge == 'true'
                        ? html`
                                <span class="ms-1 icon-pulse text-orange">
                      ...
                      ${this.is_charge == 'true'
                        ? html`
                                  ...
                                  ? html`
                                    ...
                                    `
                                  : html`
                                        <span class="btn btn-pill btn-purple" @click=${this._shopping_cart}>
                                            <span class="icon-pulse text-white">
        ...
        ? html`
          <custom-slide
            slide-title="演员阵容"
            slide-click='javascript:navmenu("discovery_person?tmdbid=${this.tmdbid}&type=${this.media_type}&title=演员&subtitle=${this.media_info.title}&pagefrom=${this.pagefrom}")'
            lazy="person-card"
            .slide_card=${this.media_info.actors.map((item) => ( html`
              <person-card
                ...
                @click=${() => {
                    console.log("演员阵容 navmenu pagefrom:"+this.pagefrom + " subtype:"+this.subtype)
                  navmenu("recommend?pagefrom="+this.subtype+"&type="+this.media_type+"&subtype=person&personid="+item.id+"&title=参演作品&subtitle="+item.name)
                }}
              ></person-card>`))
            }
          ></custom-slide>`
  ...
  _shopping_cart(e){
      e.stopPropagation();
      console.log('mediainfo index.js'+this.fourkhub_id)
      var pagefrom = this.subtype
      var title = this.media_info.title
      var year = this.media_info.year
  ...
  _loveClick(e) {
      var pagefrom = this.subtype
      var fourkhub_id = this.fourkhub_id
      //alert('订阅click，页面来源subtype:'+ subtype)
      var title = this.media_info.title
      var year = this.media_info.year
      var media_type = this.media_type
      var tmdbid = this.tmdbid
