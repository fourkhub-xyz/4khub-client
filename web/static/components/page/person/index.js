          ...
          <div class="d-grid gap-3 grid-media-card">
            ...
            ? this.person_list.map((item, index) => ( html`
              <person-card
                ...
                @click=${() => {
                    console.log("person/index.js  recommend?type 参演作品 pagefrom:" + this.pagefrom)
                  navmenu("recommend?pagefrom="+this.pagefrom+"&type="+this.media_type+"&subtype=person&personid="+item.id+"&title=参演作品&subtitle="+item.name)
                }}
              ></person-card>
              ` ) )
