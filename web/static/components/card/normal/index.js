...
export class NormalCard extends observeState(CustomElement) {
  ...
  static properties = {
    ...
    is_charge: {attribute: "card-is_charge"},
  ...
  _render_bottom() {
    if (this.show_sub == "1") {
        ...
        if (this.is_charge == 'true'){
            if (this.fav == "1") {
                return html`
                <div class="d-flex justify-content-between">
                  <a class="text-muted" title="价格${this.coins}金币" href='#'>
                    ...
                    <!--已经购买了 显示红心已订阅-->
                  <div class="ms-auto">
                    <div class="text-muted" title="已购买" style="cursor: pointer" >
                      <span class="icon-pulse text-white">
                        <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-heart icon-filled text-red" width="24" height="24"
                            viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round"
                            stroke-linejoin="round">
                          <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
                ...
                <div class="d-flex justify-content-between">
                  <a class="text-muted" title="价格${this.coins}金币" href='#'>
                    ...
                    <!-- 未购买显示购物车 -->
                  <div class="ms-auto">
                    <div class="text-muted" title="购买" style="cursor: pointer" @click=${this._shopping_cart}>
                      <span class="icon-pulse text-white">
        ...
        <div ?hidden=${cardState.more_id != this._card_id && this._card_image_error == false}
             ...
             @click=${() => { 
                 if (this.pagefrom == undefined){
                     var url = `media_detail?fourkhub_id=${encodeURIComponent(this.fourkhub_id)}&coins=${this.coins}&is_charge=${this.is_charge}&pagefrom=${this.subtype}&subtype=${this.subtype}&type=${this.media_type}&id=${encodeURIComponent(this.tmdb_id)}`
                     console.log("b pageform undefined")
                     console.log(url)
                     navmenu(url) 
                 } else {
                     if (this.subtype.length >= this.pagefrom.length){
                         if (this.tmdb_id.length === 0 || this.tmdb_id === '0'){
                             this.tmdb_id = this.fourkhub_id
                             this.subtype = '4KHUBVIP'
                         }
                         var url = `media_detail?fourkhub_id=${encodeURIComponent(this.fourkhub_id)}&coins=${this.coins}&is_charge=${this.is_charge}&pagefrom=${this.subtype}&subtype=${this.subtype}&type=${this.media_type}&id=${encodeURIComponent(this.tmdb_id)}`
                         console.log(url)
                         navmenu(url) 
                         ...
                         var url = `media_detail?fourkhub_id=${encodeURIComponent(this.fourkhub_id)}&coins=${this.coins}&is_charge=${this.is_charge}&pagefrom=${this.pagefrom}&subtype=${this.pagefrom}&type=${this.media_type}&id=${encodeURIComponent(this.tmdb_id)}`
                         navmenu(url) 
                     }
                 }
             }}>
  ...
  _shopping_cart(e){
      ...
      var tmdbid = this.tmdb_id
      var fav = this.fav
       //console.log("card/normal/index.js shopping_cart tmdbid:"+tmdbid  + " pagefrom:" + pagefrom + " fourkhub_id:"+fourkhub_id)
  ...
  _loveClick(e) {
      ...
      if (subtype === '4KHUBVIP'){
       ...
       //console.log("card/normal/index.js loveclick tmdbid:"+tmdbid + " subtype:"+subtype + " pagefrom:" + pagefrom + " fourkhub_id:"+fourkhub_id)
